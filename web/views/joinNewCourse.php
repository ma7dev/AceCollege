<?php
	...
	$coursesIn = "SELECT Courses.cID, Courses.Title, Courses.Department, Courses.CourseCode  
	              FROM Courses, Enrollment 
				  WHERE Courses.cID = Enrollment.cID AND Enrollment.uID = $user";
	$coursesResultIn = mysqli_query($conn, $coursesIn);
	if (!$coursesResultIn) {
		die("Query to show fields from table failed");
	}
	$coursesOut = "SELECT DISTINCT Courses.cID, Courses.Title, Courses.Department, Courses.CourseCode  
			       FROM Courses 
				   WHERE Courses.cID NOT IN (SELECT c.cID  
				                             FROM Courses AS c, Enrollment AS e 
											 WHERE c.cID = e.cID AND e.uID = $user)";
  ...
  <div id="site-content">
		...
		<?php if($checkOut != NULL || $checkIn != NULL){ ?>
		    <?php if($checkOut != NULL){ ?>
			    <table>
					<tr class="enroll-opt" id="<?php echo $checkOut[0]?>">
			            <th><span class="course-title"><?php echo "$checkOut[1]"?></span></th>
			            <th class="parent"><span class="course-code"><?php echo "$checkOut[2]-$checkOut[3]"?></span></th>
